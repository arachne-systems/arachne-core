...
#[cfg(test)]
mod tests {
    use crate::{emit_i64_be, parse_i64_be, parse_u16};

    buffer!(CompatibilityBuffer(14) {
        kind: (u16, 0..2),
        count: (u32, 2..6),
        delta: (i64, 6..14),
        payload: (slice, 14..),
    ...
    #[test]
    fn generated_accessors_and_parsers_preserve_bytes() {
        let mut bytes = [0u8; 16];
        let mut buffer = CompatibilityBuffer::new_checked(&mut bytes).unwrap();
        buffer.set_kind(0x1234);
        buffer.set_count(u32::MAX);
        buffer.set_delta(-42);
        buffer.payload_mut().copy_from_slice(&[7, 9]);
        assert_eq!(buffer.kind(), 0x1234);
        assert_eq!(buffer.count(), u32::MAX);
        assert_eq!(buffer.delta(), -42);
        assert_eq!(&buffer.inner_mut()[..2], &0x1234u16.to_ne_bytes());
        let buffer = CompatibilityBuffer::new(&*buffer.into_inner());
        assert_eq!(buffer.payload(), &[7, 9]);
        assert_eq!(&buffer.inner()[..2], &0x1234u16.to_ne_bytes());
        assert!(CompatibilityBuffer::new_checked(&bytes[..13]).is_err());
        assert!(parse_u16(&[0]).is_err());
        assert!(emit_i64_be(&mut [0; 7], -42).is_err());
        emit_i64_be(&mut bytes[..8], -42).unwrap();
        assert_eq!(&bytes[..8], &(-42i64).to_be_bytes());
        assert_eq!(parse_i64_be(&bytes[..8]).unwrap(), -42);
    }
}
